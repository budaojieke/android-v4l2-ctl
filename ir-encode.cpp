...
static int sanyo_encode(enum rc_proto proto, unsigned scancode, unsigned *buf)
{
	...
	auto add_bits = [&](int bits, int count)
	{
		int i;
		for (i=0; i<count; i++) {
...
static int rc5_encode(enum rc_proto proto, unsigned scancode, unsigned *buf)
{
	...
	auto advance_space = [&](unsigned length)
	{
		if (n % 2)
			buf[n] += length;
		else
	...
	auto advance_pulse = [&](unsigned length)
	{
		if (n % 2)
			buf[++n] = length;
		else
...
static int rc6_encode(enum rc_proto proto, unsigned scancode, unsigned *buf)
{
	...
	auto add_bits = [&](unsigned bits, unsigned count, unsigned length)
	{
		while (count--) {
			if (bits & (1 << count)) {
